...
4. **커밋 수행**
   ```bash
   git add src/domain/ports/outbound/mcp_client_port.py \
           src/domain/ports/outbound/hitl_notification_port.py \
           src/domain/ports/outbound/event_broadcast_port.py \
           ...
           tests/unit/fakes/test_fake_hitl_notification.py \
           tests/unit/fakes/test_fake_sse_broker.py \
           tests/unit/fakes/test_fake_orchestrator.py \
           docs/developers/architecture/layer/ports/README.md \
           tests/docs/STRATEGY.md \
           tests/docs/WritingGuide.md

   git commit -m "$(cat <<'EOF'
   feat: implement Phase 2 - Port Interface + Fake Adapters

   - Add McpClientPort with Domain callback protocols (SamplingCallback, ElicitationCallback)
   - Add HitlNotificationPort for SSE notification abstraction
   - Add EventBroadcastPort for pub/sub pattern
   - Extend OrchestratorPort with generate_response() for Method C
   - Implement FakeMcpClient with test scenario support
   - Implement FakeHitlNotification with event recording
   - Implement FakeSseBroker for testing
   - Extend FakeOrchestrator with generate_response()

   Test Coverage:
   - All Fake adapters have self-tests (TDD approach)
   - Domain callback protocols use Protocol typing for purity
   - Fake adapters support test scenario configuration
...
5. **Phase Status 업데이트**
   - `docs/project/planning/active/07_hybrid_dual/README.md`에서 Phase 2 Status를 ✅로 변경
